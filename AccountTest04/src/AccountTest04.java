...
//  -> 방법1 : 인스턴스 변수를 추가해서 생성자에서 1씩 증가함. -> 잘못된 방법
//  -> 방법2 : 클래스 변수를 추가하여 생산자에서 1씩 증가하여 인스턴스 변수에 대입함. -> 올바른 방법 

class Account {
	// 클래스 변수, static field(정적 필드)
	// 클래스에서 오직 한개만 생성되고, 모든 인스턴스에서 공유함.
	// 해당 클래스를 사용할 때 생성.
	private static int count = 0;
	
	// 인스턴스 변수, non-static field(비정적 필드)
	private int id;
	private String name;
	private String no;
	private int balance;
...
public class AccountTest04 {
	public static void main(String[] args) {
		...
		Math.
		System.out.println("발행된 은행 계좌 개수 : " + Account.getCount()); // 클래스 메소드를 올바르게 사용하는 방법, 권장하는 방법
		System.out.println("발행된 은행 계좌 개수 : " + mary.getCount()); // 틀린건 아니지만 오해의 소지가 있다. 권장하는 방법X, 인스턴스를 생성하지 않으면 에러가 발생.
	}
