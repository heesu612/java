...
/*
문제 3) 아래의 조건을 해결하고 예외를 처리
1. sm2에 m을 참조했을 때 발생하는 예외를 발견하고, 예외를 처리
2. gm2에 m을 참조했을 때 발생하는 예외를 발견하고, 예외를 처리
3. sm2에 gm을 참조했을 때 발생하는 예외를 발견하고, 예외를 처리
 */

/*
< 상속에서의 캐스팅 >
1. 업 캐스팅(Up Casting) : 자식에서 부모로 캐스팅하는 경우, 다형성을 구현
2. 다운 캐스팅(Down Casting) : 부모에서 자식으로 캐스팅하는 경우
 */
public class ExceptionTest09 {
	public static void main(String[] args) {
		...
		//sm2= (SpecialMember)gm; // 부모 자식간의 관계가 아니라 캐스팅도 불가능(같은 부모를 가졌지만 형제 관계라는건 자바에 없으므로 남남)
		
		System.out.println(m);
		System.out.println(sm);
		System.out.println(gm);
		System.out.println("프로그램에서 예외를 처리하고, 정상적으로 종료.");
	}
}
